...
כתוב תמיד את title, description ו-affected_feature באנגלית, גם אם ההודעה בעברית.`;
...
const TRIAGE_TOOL: Anthropic.Messages.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      title: { type: 'string', description: 'Concise title, always in English' },
      severity: { type: 'string', enum: ['critical', 'high', 'medium', 'low'] },
      description: { type: 'string', description: 'Detailed description, always in English' },
      steps_to_reproduce: { type: 'array', items: { type: 'string' } },
      affected_feature: { type: 'string', description: 'Affected feature name, always in English' },
